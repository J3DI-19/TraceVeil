...
class Phase3Service:
    ...
    def device_state(self, session_id: str, device_id: str, stale_after_seconds: int | None = None) -> dict:
        ...
        result = dict(row)
        metrics = json.loads(result.pop("latest_metrics_json"))
    ...
    def stream_gap(self, last_id: int, case_id: int | None, session_id: str | None) -> dict | None:
        """Detects a resumption cursor that retention has already passed.
        ...
        Otherwise returns a bounded description of the gap: how far back
        the client asked to resume from, the oldest message still held,
        and the cursor it should resume from.
    ...
    def send_email(self, draft_id: str, request_id: str | None) -> dict:
        ...
        # The delivery attempt, the draft's terminal state and the alert's
        # notification status are written in one transaction. A crash
        # between them would otherwise leave the alert list claiming a
        # notification was never requested after one was actually sent.
        with self.repository.write_lock, self.db:
        ...
        self.audit(anchor["case_id"], f"email.{status}", "email_draft", draft_id, request_id=request_id, details={"message_id": message["Message-ID"], "subject_type": anchor["subject_type"], "subject_id": draft["subject_id"], "notified_alert_ids": notified})
        return self.get_email_draft(draft_id)
