...
# ---------------------------------------------------------------------------
# Device connection state: online, stale and offline are distinct facts
# ---------------------------------------------------------------------------
def _device(env, case_id: int, session_id: str | None = None) -> dict:
    query = f"?session_id={session_id}" if session_id else ""
    items = env.http.get(f"{API}/cases/{case_id}/live/devices{query}").json()["items"]
    assert items, "no device state was persisted"
    return items[0]


def _telemetry(env, case_id: int, sequence: int, metrics: dict, observed: str) -> None:
    payload = {"schema_version": "1.0", "case_id": case_id, "source_id": SOURCE,
               "device_id": "door-controller", "event_type": "heartbeat",
               "observed_at": observed, "sequence": sequence, "metrics": metrics}
    accepted = env.http.post(f"{API}/live/telemetry", json=payload,
                               headers={"X-Traceveil-Source-Token": VALID_TOKEN})
    assert accepted.status_code == 202, accepted.text
    assert _wait_receipt(env, accepted.json()["receipt_id"])["status"] == "completed"


def _backdate_last_seen(env, session_id: str, seconds: int) -> None:
    """Age a device's last accepted record without waiting for the clock.

    The staleness boundary is what is under test, not the passage of
    time, and a test that slept past a 300-second maximum threshold
    would be untestable.
    """
    service = _service(env)
    aged = (datetime.now(timezone.utc) - timedelta(seconds=seconds)).isoformat()
    with service.repository.write_lock, service.db:
        service.db.execute("UPDATE device_states SET last_seen_at=? WHERE session_id=?",
                           (aged, session_id))


def test_device_becomes_stale_after_its_own_session_threshold(client):
    """The threshold belongs to the session, not to a global default.
    A silent device must age out on the boundary its session declared."""
    env = _env(client)
    cid = _case(env, "device staleness")
    started = env.http.post(f"{API}/cases/{cid}/live-sessions",
                            json={"label": "device states", "source_ids": [SOURCE],
                                  "stale_after_seconds": 120}).json()
    _telemetry(env, cid, 0, {"state": "ok"}, "2026-09-15T10:00:00Z")

    fresh = _device(env, cid, started["session_id"])
    assert fresh["connection_state"] == "online"
    assert fresh["stale_after_seconds"] == 120

    # Still inside the declared window.
    _backdate_last_seen(env, started["session_id"], 60)
    assert _device(env, cid, started["session_id"])["connection_state"] == "online"

    _backdate_last_seen(env, started["session_id"], 180)
    aged = _device(env, cid, started["session_id"])
    assert aged["connection_state"] == "stale", "a silent device is still reported Online"
    assert aged["stale"] is True


def test_two_sessions_with_different_thresholds_transition_differently(client):
    """A session that declared a long threshold must not inherit a short
    one from another session, and vice versa."""
    env = _env(client)
    patient = _case(env, "patient session")
    impatient = _case(env, "impatient session")
    slow = env.http.post(f"{API}/cases/{patient}/live-sessions",
                         json={"label": "patient", "source_ids": [SOURCE],
                               "stale_after_seconds": 300}).json()
    _telemetry(env, patient, 0, {"state": "ok"}, "2026-09-15T10:00:00Z")
    env.http.post(f"{API}/live-sessions/{slow['session_id']}/stop")

    quick = env.http.post(f"{API}/cases/{impatient}/live-sessions",
                          json={"label": "impatient", "source_ids": [SOURCE],
                                "stale_after_seconds": 5}).json()
    _telemetry(env, impatient, 1, {"state": "ok"}, "2026-09-15T10:00:01Z")

    # The same elapsed silence for both: only the declared thresholds differ.
    _backdate_last_seen(env, slow["session_id"], 60)
    _backdate_last_seen(env, quick["session_id"], 60)

    assert _device(env, patient, slow["session_id"])["connection_state"] == "online"
    assert _device(env, impatient, quick["session_id"])["connection_state"] == "stale"


def test_reported_link_loss_renders_offline_despite_a_recent_timestamp(client):
    """A retained transport message announcing a dropped link carries a
    fresh timestamp. Recency alone would show a disconnected sensor as
    Online, which is the misreading this state exists to prevent."""
    env = _env(client)
    cid = _case(env, "link loss")
    session = env.http.post(f"{API}/cases/{cid}/live-sessions",
                            json={"label": "link", "source_ids": [SOURCE],
                                  "stale_after_seconds": 300}).json()
    _telemetry(env, cid, 0, {"link_status": "offline"}, "2026-09-15T10:00:00Z")

    device = _device(env, cid, session["session_id"])
    assert device["connection_state"] == "offline"
    assert device["stale"] is True, "offline must not read as currently reporting"


def test_a_valid_event_returns_a_device_to_online(client):
    env = _env(client)
    cid = _case(env, "link recovery")
    session = env.http.post(f"{API}/cases/{cid}/live-sessions",
                            json={"label": "recovery", "source_ids": [SOURCE],
                                  "stale_after_seconds": 300}).json()
    _telemetry(env, cid, 0, {"link_status": "offline"}, "2026-09-15T10:00:00Z")
    assert _device(env, cid, session["session_id"])["connection_state"] == "offline"

    _telemetry(env, cid, 1, {"link_status": "connected"}, "2026-09-15T10:00:05Z")
    assert _device(env, cid, session["session_id"])["connection_state"] == "online"


def test_reading_device_status_changes_nothing(client):
    """Status is derived at read time. Re-deriving it must not touch
    evidence, canonical events or analysis artifacts."""
    env = _env(client)
    cid = _case(env, "device status invariance")
    session = env.http.post(f"{API}/cases/{cid}/live-sessions",
                            json={"label": "invariance", "source_ids": [SOURCE],
                                  "stale_after_seconds": 30}).json()
    _telemetry(env, cid, 0, {"state": "ok"}, "2026-09-15T10:00:00Z")
    _settle(env, cid)

    before = _forensic_snapshot(env, cid)
    for _ in range(5):
        env.http.get(f"{API}/cases/{cid}/live/devices")
    _backdate_last_seen(env, session["session_id"], 120)
    for _ in range(5):
        assert env.http.get(f"{API}/cases/{cid}/live/devices").json()["items"][0]["connection_state"] == "stale"
    assert _forensic_snapshot(env, cid) == before


# ---------------------------------------------------------------------------
# Reconnect: a retention gap must never be silent
# ---------------------------------------------------------------------------
def test_cursor_inside_retention_is_not_a_gap(client):
    env = _env(client)
    cid = _case(env, "retained cursor")
    sid = _session(env, cid)
    _ingest(env, cid, 2)
    _settle(env, cid)
    ids = [message["stream_id"] for message in _messages(env, cid, sid)]

    service = _service(env)
    assert service.stream_gap(0, cid, sid) is None, "a fresh connection is not a gap"
    assert service.stream_gap(ids[0], cid, sid) is None
    assert service.stream_gap(ids[-1], cid, sid) is None


def _prune_stream_below(env, keep_from: int) -> None:
    """Simulate retention having already discarded the oldest messages."""
    service = _service(env)
    with service.repository.write_lock, service.db:
        service.db.execute("DELETE FROM stream_messages WHERE stream_id < ?", (keep_from,))


def test_pruned_cursor_is_reported_as_an_explicit_gap(client):
    """Once retention has passed a cursor, resuming after it silently
    drops every message in between. The client is told instead."""
    env = _env(client)
    cid = _case(env, "pruned cursor")
    sid = _session(env, cid)
    _ingest(env, cid, 3)
    _settle(env, cid)
    service = _service(env)
    ids = sorted(message["stream_id"] for message in _messages(env, cid, sid))
    assert len(ids) >= 3, ids

    stale_cursor = ids[0]
    _prune_stream_below(env, ids[-1])

    gap = service.stream_gap(stale_cursor, cid, sid)
    assert gap is not None, "an expired cursor was accepted as complete"
    assert gap["reason"] == "cursor_expired"
    assert gap["earliest_retained"] == ids[-1]
    # Resuming from just before the oldest retained message means the
    # recovery itself skips nothing.
    assert gap["resume_from"] == ids[-1] - 1
    assert {"devices", "metrics", "timeline", "events", "alerts"} <= set(gap["resynchronize"])
    # A cursor still inside the surviving window is unaffected.
    assert service.stream_gap(ids[-1], cid, sid) is None


def test_expired_cursor_yields_a_stream_reset_frame(live_server):
    """The gap is announced on the wire, before any later message."""
    cid = _case(live_server, "reset frame")
    sid = _session(live_server, cid)
    _ingest(live_server, cid, 3)
    _settle(live_server, cid)
    ids = sorted(m["stream_id"] for m in _messages(live_server, cid, sid))
    stale_cursor = ids[0]
    _prune_stream_below(live_server, ids[-1])

    events = _read_sse(
        live_server, f"{STREAM_PATH}?case_id={cid}&session_id={sid}",
        headers={"Last-Event-ID": str(stale_cursor)}, max_events=2)

    assert events[0]["event"] == "stream.reset", [event.get("event") for event in events]
    assert events[0]["data"]["payload"]["reason"] == "cursor_expired"
    assert events[0]["data"]["schema_version"] == "1.0"
    # Everything after the reset is ordinary delivery resuming from the
    # server-provided cursor, so no retained message is lost to recovery.
    assert all(event["id"] >= ids[-1] for event in events[1:])


def test_a_retained_cursor_produces_no_reset(live_server):
    cid = _case(live_server, "no reset")
    sid = _session(live_server, cid)
    _ingest(live_server, cid, 2)
    _settle(live_server, cid)
    ids = [m["stream_id"] for m in _messages(live_server, cid, sid)]

    events = _read_sse(
        live_server, f"{STREAM_PATH}?case_id={cid}&session_id={sid}",
        headers={"Last-Event-ID": str(ids[0])}, max_events=2)
    assert all(event.get("event") != "stream.reset" for event in events)


def test_cursor_state_cannot_leak_across_cases(client):
    """Stream ids are one global sequence, so a quiet case must not be
    told it has a gap merely because another case produced the messages
    its cursor sits behind."""
    env = _env(client)
    first = _case(env, "cursor case A")
    second = _case(env, "cursor case B")
    first_session = _session(env, first)
    _ingest(env, first, 2)
    _settle(env, first)
    service = _service(env)
    highest = max(message["stream_id"] for message in _messages(env, first, first_session))

    # Case B produced nothing, yet a client watching it is up to date with
    # the retained window, so no reset is owed.
    assert service.stream_gap(highest, second, None) is None
    assert _messages(env, second, None) == []


def test_stream_reset_leaves_persisted_records_untouched(live_server):
    cid = _case(live_server, "reset invariance")
    sid = _session(live_server, cid)
    _ingest(live_server, cid, 2)
    _settle(live_server, cid)
    before = _forensic_snapshot(live_server, cid)

    _read_sse(live_server, f"{STREAM_PATH}?case_id={cid}&session_id={sid}",
              headers={"Last-Event-ID": "1"}, max_events=1)
    _read_sse(live_server, f"{STREAM_PATH}?case_id={cid}&session_id={sid}",
              headers={"Last-Event-ID": "1"}, max_events=1)
    _settle(live_server, cid)
    assert _forensic_snapshot(live_server, cid) == before
...
def test_cursor_ahead_of_the_log_is_rejected(client):
    """A cursor naming ids the server never issued cannot be resumed from:
    no future id ever reaches it, so the stream would stay silent forever
    rather than fail. It is refused with the same reset instruction."""
    env = _env(client)
    cid = _case(env, "cursor ahead")
    sid = _session(env, cid)
    _ingest(env, cid, 2)
    _settle(env, cid)
    service = _service(env)
    latest = max(message["stream_id"] for message in _messages(env, cid, sid))

    gap = service.stream_gap(latest + 500, cid, sid)
    assert gap is not None, "a cursor beyond the log was accepted"
    assert gap["reason"] == "cursor_ahead"
    assert gap["resume_from"] == latest
    assert {"devices", "metrics", "timeline", "events", "alerts"} <= set(gap["resynchronize"])
    # The newest retained cursor itself is still perfectly valid.
    assert service.stream_gap(latest, cid, sid) is None


def test_malformed_cursor_is_rejected_rather_than_replayed(client):
    """An unparseable Last-Event-ID means the client believes it has
    history. Treating it as a fresh connection would silently replay."""
    service = _service(_env(client))
    gap = service.stream_gap(None, None, None)
    assert gap is not None
    assert gap["reason"] == "cursor_invalid"
    assert gap["resume_from"] == 0
    assert {"devices", "metrics", "timeline", "events", "alerts"} <= set(gap["resynchronize"])


def test_malformed_cursor_header_yields_a_reset_not_an_error(live_server):
    """Over real HTTP a malformed header must not become a 4xx the
    browser's reconnect loop would retry against forever."""
    cid = _case(live_server, "malformed cursor")
    sid = _session(live_server, cid)
    _ingest(live_server, cid, 1)
    _settle(live_server, cid)

    events = _read_sse(live_server, f"{STREAM_PATH}?case_id={cid}&session_id={sid}",
                       headers={"Last-Event-ID": "not-a-number"}, max_events=1)
    assert events[0]["event"] == "stream.reset"
    assert events[0]["data"]["payload"]["reason"] == "cursor_invalid"


def test_cursor_ahead_yields_a_reset_frame_over_http(live_server):
    cid = _case(live_server, "ahead cursor frame")
    sid = _session(live_server, cid)
    _ingest(live_server, cid, 1)
    _settle(live_server, cid)
    latest = max(m["stream_id"] for m in _messages(live_server, cid, sid))

    events = _read_sse(live_server, f"{STREAM_PATH}?case_id={cid}&session_id={sid}",
                       headers={"Last-Event-ID": str(latest + 1000)}, max_events=1)
    assert events[0]["event"] == "stream.reset"
    assert events[0]["data"]["payload"]["reason"] == "cursor_ahead"


def test_persisted_events_and_alerts_survive_a_reset(client):
    """Reset recovery reads persisted records, so those records must be
    there to read. Rebuilding is only meaningful if the backend kept them."""
    env = _env(client)
    cid = _case(env, "reset recovery source")
    _session(env, cid)
    _ingest(env, cid, 2)
    _settle(env, cid)

    events = env.http.get(f"{API}/cases/{cid}/events").json()
    assert events["items"], "no persisted events for reset recovery to reload"
    # The alert register is queryable even when empty; both are what the
    # browser refetches instead of trusting its display buffers.
    assert "items" in env.http.get(f"{API}/cases/{cid}/alerts").json()


def test_one_unreadable_metrics_blob_does_not_fault_the_device_listing(client):
    """The device register is also written by transport bridges. A single
    malformed or absent metrics blob must degrade to an empty reading, not
    take every device's status down with it."""
    env = _env(client)
    cid = _case(env, "unreadable metrics")
    session = env.http.post(f"{API}/cases/{cid}/live-sessions",
                            json={"label": "bridge", "source_ids": [SOURCE],
                                  "stale_after_seconds": 300}).json()
    _telemetry(env, cid, 0, {"state": "ok"}, "2026-09-15T10:00:00Z")
    service = _service(env)

    # NULL is refused by the schema, so these are the shapes that can
    # actually reach the column: empty, unparseable, or valid JSON that
    # simply is not an object.
    for corrupt in ("", "   ", "not json", "[1,2,3]", "null"):
        with service.repository.write_lock, service.db:
            service.db.execute("UPDATE device_states SET latest_metrics_json=? WHERE session_id=?",
                               (corrupt, session["session_id"]))
        response = env.http.get(f"{API}/cases/{cid}/live/devices")
        assert response.status_code == 200, f"{corrupt!r} faulted the listing: {response.text}"
        device = response.json()["items"][0]
        assert device["latest_metrics"] == {}
        # Status is still reported, because it does not depend on metrics.
        assert device["connection_state"] in {"online", "stale", "offline"}
