...
# Budgets, not delays: every loop below exits the moment the backend is
# ready, so a generous ceiling costs a fast machine nothing. They are sized
# for a shared two-core CI runner, where the worker normalizes, persists and
# re-analyses each record far slower than a development machine does.
...
def _wait_receipt(client, receipt_id: str) -> dict:
    deadline = monotonic() + RECEIPT_TIMEOUT_S
    ...
    raise AssertionError(
        f"receipt {receipt_id} never reached a terminal status within {RECEIPT_TIMEOUT_S:g}s")
...
def _case_with_alert(client) -> tuple[int, dict]:
    ...
    deadline = monotonic() + ALERT_TIMEOUT_S
    while monotonic() < deadline:
        ...
        sleep(POLL_INTERVAL_S)
    raise AssertionError(
        f"AUTH-001 alert never appeared within {ALERT_TIMEOUT_S:g}s of ten authentication failures")
...
# ---------------------------------------------------------------------------
# Terminal delivery state and its audit record commit together
# ---------------------------------------------------------------------------
def _approved_alert_draft(client, monkeypatch, sent: list | None = None) -> tuple[int, dict, dict]:
    cid, alert = _case_with_alert(client)
    draft = client.post(f"/api/v1/cases/{cid}/alerts/{alert['alert_id']}/email-drafts",
                        json={"recipient": RECIPIENT}).json()
    client.post(f"/api/v1/email-drafts/{draft['draft_id']}/approve",
                json={"approver": "Investigator", "confirmed": True})
    _configure_smtp(client, sent if sent is not None else [], monkeypatch)
    return cid, alert, draft


def _counts(client, draft_id: str, case_id: int, alert_id: str) -> dict:
    db = client.app.state.phase3_service.db
    return {
        "attempts": db.execute("SELECT COUNT(*) FROM delivery_attempts WHERE draft_id=?", (draft_id,)).fetchone()[0],
        "sent_at": db.execute("SELECT sent_at FROM email_drafts WHERE draft_id=?", (draft_id,)).fetchone()[0],
        "status": db.execute("SELECT status FROM email_drafts WHERE draft_id=?", (draft_id,)).fetchone()[0],
        "notification": (db.execute(
            "SELECT notification_status FROM alert_workflow WHERE case_id=? AND alert_id=?",
            (case_id, alert_id)).fetchone() or ["absent"])[0],
        "audits": db.execute(
            "SELECT COUNT(*) FROM audit_events WHERE subject_id=? AND action LIKE 'email.%'",
            (draft_id,)).fetchone()[0],
    }


def test_a_failing_terminal_audit_rolls_back_the_whole_delivery(client, monkeypatch):
    """The audit record and the state it describes must land together.

    If the audit insert can fail on its own, a completed delivery can
    exist with no history of it - which is precisely what an audit trail
    is for. Forcing that insert to fail must leave no delivery attempt,
    no terminal draft state and no alert notification status behind.
    """
    sent: list = []
    cid, alert, draft = _approved_alert_draft(client, monkeypatch, sent)
    service = client.app.state.phase3_service
    before = _counts(client, draft["draft_id"], cid, alert["alert_id"])

    def failing_audit(*args, **kwargs):
        raise sqlite3.OperationalError("simulated audit failure")
    monkeypatch.setattr(service, "_audit_locked", failing_audit)

    with pytest.raises(sqlite3.OperationalError):
        service.send_email(draft["draft_id"], "request-rollback")
    monkeypatch.undo()

    after = _counts(client, draft["draft_id"], cid, alert["alert_id"])
    assert after == before, "a failed terminal audit left delivery state behind"
    assert after["attempts"] == 0
    assert after["sent_at"] is None
    assert after["notification"] in {"absent", "not_requested"}
    # The message itself did leave through SMTP - that cannot be undone -
    # so the draft must remain approved and re-sendable rather than
    # recorded as delivered without evidence.
    assert after["status"] == "approved"


def test_successful_delivery_commits_exactly_one_of_each_record(client, monkeypatch):
    sent: list = []
    cid, alert, draft = _approved_alert_draft(client, monkeypatch, sent)
    ...
    assert after["notification"] == "delivered"
    assert after["audits"] == 1
    assert len(sent) == 1


def test_uncertain_delivery_commits_its_failed_state_once(client, monkeypatch):
    cid, alert = _case_with_alert(client)
    draft = client.post(f"/api/v1/cases/{cid}/alerts/{alert['alert_id']}/email-drafts",
                        json={"recipient": RECIPIENT}).json()
    client.post(f"/api/v1/email-drafts/{draft['draft_id']}/approve",
    ...
    after = _counts(client, draft["draft_id"], cid, alert["alert_id"])
    assert after["attempts"] == 1
    assert after["status"] == "delivery_unknown"
    assert after["notification"] == "delivery_failed"
    assert after["audits"] == 1


def test_repeated_send_adds_no_second_attempt_or_audit(client, monkeypatch):
    sent: list = []
    cid, alert, draft = _approved_alert_draft(client, monkeypatch, sent)
    client.post(f"/api/v1/email-drafts/{draft['draft_id']}/send")
    first = _counts(client, draft["draft_id"], cid, alert["alert_id"])
    client.post(f"/api/v1/email-drafts/{draft['draft_id']}/send")
    client.post(f"/api/v1/email-drafts/{draft['draft_id']}/send")

    assert _counts(client, draft["draft_id"], cid, alert["alert_id"]) == first
    assert len(sent) == 1


def test_terminal_audit_details_stay_bounded(client, monkeypatch):
    """Identifiers, hashes, reason codes and a message id only - never a
    recipient, a message body, an attachment or an SMTP transcript."""
    sent: list = []
    cid, _, draft = _approved_alert_draft(client, monkeypatch, sent)
    client.post(f"/api/v1/email-drafts/{draft['draft_id']}/send")

    audit = client.get(f"/api/v1/cases/{cid}/audit").json()["items"]
    terminal = next(item for item in audit if item["action"] == "email.sent")
    assert set(terminal["details"]) == {"message_id", "subject_type", "subject_id", "notified_alert_ids"}
    serialized = json.dumps(audit)
