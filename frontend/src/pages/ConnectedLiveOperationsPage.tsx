...
export function ConnectedLiveOperationsPage({ navigate, search = "" }: { navigate: (path: string) => void; search?: string }) {
  ...
  // The outstanding promise to report the timeline resource of a recovery.
  // It belongs to the recovery, not to any one request, because a request
  // can be superseded before it answers.
  const timelineObligation = useRef<Scope | null>(null);
  ...
  useEffect(() => {
    ...
    timelineObligation.current = null;
  ...
  const refreshTimeline = (_signal?: AbortSignal, analysisId?: string, recoveryScope?: Scope) => {
    ...
    if (!scope) return;
    // A recovery's timeline request can be superseded before it answers -
    // a `timeline.updated` frame during recovery aborts it and starts a
    // newer one. The obligation to report that resource therefore moves
    // to whichever request is now live, instead of dying with the aborted
    // one and leaving the rebuild stuck on "still loading: timeline"
    // forever. It is never satisfied by the abort itself: only a request
    // that actually answers can settle it.
    if (recoveryScope) timelineObligation.current = recoveryScope;
    else if (timelineObligation.current && !stillCurrent(timelineObligation.current)) timelineObligation.current = null;
    const obligation = timelineObligation.current;
    ...
    /** Settle the recovery obligation, if this request still carries it. */
    const settleObligation = (ok: boolean) => {
      if (!obligation || timelineObligation.current !== obligation) return;
      timelineObligation.current = null;
      reportRecovery(obligation, "timeline", ok);
    };
    void investigationApi.timeline(scope.caseId, { page: 1, pageSize: MAX_TIMELINE, analysisId: analysisId ?? null }, controller.signal)
      .then(result => {
        ...
        settleObligation(true);
      })
      .catch(reason => {
        // A superseded request is not a failure, and must never surface an
        // error or blank the chronology. It also must not settle the
        // obligation: the request that replaced it inherited it and will.
        if (controller.signal.aborted) return;
        if (stillCurrent(scope) && requestId === timelineSequence.current) {
          setError(normalizeApiError(reason).message);
        }
        settleObligation(false);
      })
