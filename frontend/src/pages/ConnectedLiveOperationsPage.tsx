...
export function ConnectedLiveOperationsPage({ navigate, search = "" }: { navigate: (path: string) => void; search?: string }) {
  ...
  const refreshTimeline = (_signal?: AbortSignal, analysisId?: string) => {
    ...
    void investigationApi.timeline(scope.caseId, { page: 1, pageSize: MAX_TIMELINE, analysisId: analysisId ?? null }, controller.signal)
      .then(result => {
        if (!stillCurrent(scope) || requestId !== timelineSequence.current) return;
  ...
  const refreshDevices = () => {
    ...
    void phase3Api.devices(scope.caseId, scope.sessionId, scope.signal)
      .then(result => { if (stillCurrent(scope)) setDevices(Array.isArray(result.items) ? result.items : []); })
      .catch(reason => { if (stillCurrent(scope)) setError(normalizeApiError(reason).message); });
  };
  const refreshMetrics = () => {
    const scope = captureScope();
    if (!scope) return;
    void phase3Api.metrics(scope.caseId, scope.sessionId, scope.signal)
      .then(result => { if (stillCurrent(scope)) setMetrics(result); })
      .catch(reason => { if (stillCurrent(scope)) setError(normalizeApiError(reason).message); });
  };
  ...
  const refreshPersistedFeeds = (scope: Scope | null) => {
    ...
    void investigationApi.events(scope.caseId, { page: 1, pageSize: EVENT_PAGE_SIZE, order: RECOVERY_ORDER }, scope.signal)
      .then(result => { if (stillCurrent(scope)) setEvents(Array.isArray(result.items) ? result.items as unknown as Record<string, unknown>[] : []); })
      .catch(reason => { if (stillCurrent(scope)) setError(`Persisted events could not be reloaded: ${normalizeApiError(reason).message}`); });
    void investigationApi.alerts(scope.caseId, { page: 1, pageSize: ALERT_PAGE_SIZE, order: RECOVERY_ORDER }, scope.signal)
      .then(result => { if (stillCurrent(scope)) setAlerts(Array.isArray(result.items) ? result.items as unknown as Record<string, unknown>[] : []); })
      .catch(reason => { if (stillCurrent(scope)) setError(`Persisted alerts could not be reloaded: ${normalizeApiError(reason).message}`); });
  };
  ...
  const resynchronize = () => {
    ...
    clearBuffers();
    refreshDevices(); refreshMetrics(); refreshTimeline(); refreshPersistedFeeds(scope);
  };
