...
function stubFetch(options: { frames?: string[]; lateFrames?: string[]; timelinePages?: unknown[]; timelineDelays?: number[]; devicePages?: Record<string, unknown>[][]; sessionIdSwitches?: boolean; eventHold?: Promise<unknown>; alertHold?: Promise<unknown> } = {}) {
...
describe("ConnectedLiveOperationsPage - real-time delivery", () => {
  ...
  it("reports a failed reload instead of an empty rebuild", async () => {
    ...
    const harness = stubFetch({
      frames: [RESET(4)],
      eventHold: Promise.reject(new Error("network down")) as unknown as Promise<unknown>,
    });
  ...
  it("discards a delayed alert response from the previous case", async () => {
    ...
    stubFetch({ frames: [RESET(4)], alertHold: held });
  ...
  it("discards a delayed event response from the previous case", async () => {
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    stubFetch({ frames: [RESET(4)], eventHold: held });
  ...
  it("discards a stale rejection instead of blaming the current case", async () => {
    // An old failure must not raise a notice against the scope that
    // replaced it.
    const rejection = Promise.reject(new Error("case four is gone"));
    rejection.catch(() => undefined);
    stubFetch({ frames: [RESET(4)], alertHold: rejection as unknown as Promise<unknown> });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    fireEvent.change(await screen.findByLabelText("Live case"), { target: { value: String(OTHER_CASE_ID) } });
    await waitFor(() => expect((screen.getByLabelText("Live case") as HTMLSelectElement).value).toBe(String(OTHER_CASE_ID)));
    await new Promise(resolve => setTimeout(resolve, 80));
  ...
  it("discards a response from a previous live session on the same case", async () => {
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    stubFetch({ sessionIdSwitches: true, frames: [RESET(4)], alertHold: held });
  ...
  it("keeps the newer recovery authoritative when an older one resolves late", async () => {
    ...
    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    // Let the second reset supersede the first.
    await new Promise(resolve => setTimeout(resolve, 120));
