...
/** Mirrors MAX_ALERTS in the page: the bound the recovery buffer holds. */
const MAX_ALERTS_BOUND = 100;
...
function stubFetch(options: { frames?: string[]; lateFrames?: string[]; timelinePages?: unknown[]; timelineDelays?: number[]; timelineHoldUnpinned?: boolean; devicePages?: Record<string, unknown>[][]; sessionIdSwitches?: boolean; eventHold?: Promise<unknown>; alertHold?: Promise<unknown>; deviceHold?: Promise<unknown>; metricsHold?: Promise<unknown>; eventFails?: boolean; alertFails?: boolean; failAll?: boolean } = {}) {
  ...
  let metricsCalls = 0;
  let sessionCalls = 0;
  const timelineHoldUnpinned = options.timelineHoldUnpinned ?? false;
  const deviceHold = options.deviceHold;
  const metricsHold = options.metricsHold;
  ...
  /** A request that never answers on its own and rejects the way a real
   *  transport does when its AbortSignal fires. Holding a response without
   *  honouring abort hides every defect in supersession handling - which is
   *  exactly how a stuck recovery status reached review. */
  const abortable = (signal: AbortSignal | null | undefined) => new Promise<never>((_resolve, reject) => {
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes("/timeline")) {
      if (failAll) { timelineCalls += 1; return Promise.reject(new Error("timeline unavailable")); }
      const pinned = new URL(url, "http://traceveil.test").searchParams.has("analysis_id");
      if (timelineHoldUnpinned && !pinned) { timelineCalls += 1; return abortable(init?.signal); }
    ...
    if (url.includes("/live/metrics")) {
      metricsCalls += 1;
      if (failAll) return Promise.reject(new Error("metrics unavailable"));
      if (metricsHold && metricsCalls === 1) return metricsHold.then(body => json(body));
    ...
    if (url.includes("/events")) {
      ...
      if (failAll) return Promise.reject(new Error("events unavailable"));
    ...
    if (url.includes("/alerts")) {
      ...
      const size = Number(new URL(url, "http://traceveil.test").searchParams.get("page_size") ?? "50");
      if (size > 200) return new Response(JSON.stringify({ code: "request_validation_error", message: "Input should be less than or equal to 200", retryable: false }), { status: 422, headers: { "Content-Type": "application/json" } });
      if (failAll) return Promise.reject(new Error("alerts unavailable"));
    ...
    if (url.includes("/live/devices")) {
      if (failAll) { deviceCalls += 1; return Promise.reject(new Error("devices unavailable")); }
      const state = deviceStates[Math.min(deviceCalls, deviceStates.length - 1)];
      deviceCalls += 1;
      if (deviceHold && deviceCalls === 1) return deviceHold.then(body => json(body));
...
describe("ConnectedLiveOperationsPage - real-time delivery", () => {
  ...
  // -------------------------------------------------------------------------
  // Records delivered during recovery must survive the snapshot
  // -------------------------------------------------------------------------
  it("keeps an event streamed while the snapshot was loading", async () => {
    // The stream stays connected during recovery, so a newer event can
    // arrive before the snapshot lands. Replacing the feed with the
    // snapshot would erase a record the backend had already delivered.
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({
      frames: [RESET(4)],
      lateFrames: [sseFrame(90, "event.accepted", { event_id: "EV-POST-RESET", observed_at: "2026-09-18T10:05:00Z", event_type: "telemetry" })],
      eventHold: held,
    });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.eventCalls()).toBeGreaterThan(0));
    // Delivered and visible before the snapshot resolves.
    expect(await screen.findByText("EV-POST-RESET")).toBeTruthy();

    release({ items: [{ event_id: "EV-SNAPSHOT-OLD", observed_at: "2026-09-18T09:00:00Z", event_type: "telemetry" }], page: 1, page_size: 1, total: 1 });
    await settle(80);

    // Both survive: the snapshot is authoritative history, the streamed
    // record is newer than the snapshot was.
    expect(screen.queryByText("EV-POST-RESET")).not.toBeNull();
    expect(screen.queryByText("EV-SNAPSHOT-OLD")).not.toBeNull();
  });

  it("keeps an alert streamed while the snapshot was loading", async () => {
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({
      frames: [RESET(4)],
      lateFrames: [sseFrame(91, "alert.created", { alert_id: "AL-POST-RESET", severity: "critical", title: "NEW POST RESET ALERT", triggered_at: "2026-09-18T10:05:00Z" })],
      alertHold: held,
    });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.alertCalls()).toBeGreaterThan(0));
    expect(await screen.findByText("NEW POST RESET ALERT")).toBeTruthy();

    release({ items: [{ alert_id: "AL-SNAPSHOT-OLD", severity: "low", title: "OLDER SNAPSHOT ALERT", triggered_at: "2026-09-18T09:00:00Z" }], page: 1, page_size: 1, total: 1 });
    await settle(80);

    expect(screen.queryByText("NEW POST RESET ALERT")).not.toBeNull();
    expect(screen.queryByText("OLDER SNAPSHOT ALERT")).not.toBeNull();
  });

  it("shows one row when the snapshot and the stream carry the same id", async () => {
    // Overlap is expected: the snapshot may already contain the record the
    // stream just delivered. Concatenating would show it twice.
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({
      frames: [RESET(4)],
      lateFrames: [sseFrame(92, "event.accepted", { event_id: "EV-OVERLAP", observed_at: "2026-09-18T10:05:00Z", event_type: "streamed-representation" })],
      eventHold: held,
    });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.eventCalls()).toBeGreaterThan(0));
    await screen.findByText("EV-OVERLAP");

    release({ items: [{ event_id: "EV-OVERLAP", observed_at: "2026-09-18T10:05:00Z", event_type: "snapshot-representation" }], page: 1, page_size: 1, total: 1 });
    await settle(80);

    expect(screen.getAllByText("EV-OVERLAP")).toHaveLength(1);
    // The streamed view wins: it is the later representation of that id.
    expect(screen.queryByText("streamed-representation")).not.toBeNull();
    expect(screen.queryByText("snapshot-representation")).toBeNull();
  });

  it("does not let a superseded snapshot merge into the newer recovery", async () => {
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({
      frames: [RESET(4)],
    ...
    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.eventCalls()).toBeGreaterThan(1));

    release({ items: [{ event_id: "EV-FROM-SUPERSEDED", observed_at: "2026-09-18T08:00:00Z", event_type: "telemetry" }], page: 1, page_size: 1, total: 1 });
    await settle(80);

    expect(screen.queryByText("EV-FROM-SUPERSEDED")).toBeNull();
  });

  // -------------------------------------------------------------------------
  // Recovery status must be truthful while requests are still in flight
  // -------------------------------------------------------------------------
  it("does not claim success while a recovery request is pending", async () => {
    // The old notice said "refetched authoritative state" before the
    // requests had returned, so a stalled request left an incomplete panel
    // paired with a completion message.
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({ frames: [RESET(4)], eventHold: held });
    ...
    expect(await screen.findByText(/rebuilding from persisted state/i)).toBeTruthy();
    expect(screen.queryByText(/Rebuilt from persisted state/i)).toBeNull();

    release({ items: [], page: 1, page_size: 0, total: 0 });
    await waitFor(() => expect(screen.queryByText(/Rebuilt from persisted state/i)).not.toBeNull());
  });

  it("reports a partial rebuild and offers a retry when a resource fails", async () => {
    const harness = stubFetch({ frames: [RESET(4)], eventFails: true });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.eventCalls()).toBeGreaterThan(0));

    expect(await screen.findByText(/rebuild finished only in part|rebuild failed/i)).toBeTruthy();
    expect(screen.queryByText(/Rebuilt from persisted state/i)).toBeNull();
    const retry = await screen.findByRole("button", { name: "Retry rebuild" });

    fireEvent.click(retry);
    // The retry issues fresh requests, and the second attempt succeeds.
    await waitFor(() => expect(harness.eventCalls()).toBeGreaterThan(1));
    await waitFor(() => expect(screen.queryByText(/Rebuilt from persisted state/i)).not.toBeNull());
  });

  it("drops recovery status when the case changes mid-rebuild", async () => {
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({ frames: [RESET(4)], eventHold: held });
    ...
    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.eventCalls()).toBeGreaterThan(0));
    await screen.findByText(/rebuilding from persisted state/i);

    await selectCase(OTHER_CASE_ID, "Other case");
    release({ items: [{ event_id: "EV-OLD-RECOVERY", observed_at: "2026-09-18T08:00:00Z", event_type: "telemetry" }], page: 1, page_size: 1, total: 1 });
    await settle(80);

    // No status at all from the abandoned rebuild, and none of its records.
    expect(screen.queryByText(/rebuilding from persisted state/i)).toBeNull();
    expect(screen.queryByText(/Rebuilt from persisted state/i)).toBeNull();
    expect(screen.queryByText("EV-OLD-RECOVERY")).toBeNull();
  });

  // -------------------------------------------------------------------------
  // The remaining resources obey the same scope rule
  // -------------------------------------------------------------------------
  it("discards a delayed device response from the previous case", async () => {
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({ frames: [RESET(4)], deviceHold: held });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.deviceCalls()).toBeGreaterThan(0));
    await selectCase(OTHER_CASE_ID, "Other case");

    release({ items: [{ case_id: CASE_ID, session_id: SESSION_ID, device_id: "OLD-CASE-DEVICE", source_id: "live-lab-01", last_seen_at: "2026-09-18T10:00:00Z", last_observed_at: "2026-09-18T10:00:00Z", latest_metrics: {}, event_count: 1, stale: false, connection_state: "online", stale_after_seconds: 30 }], page: 1, page_size: 1, total: 1 });
    await settle(80);

    expect(screen.queryByText("OLD-CASE-DEVICE")).toBeNull();
  });

  it("discards a delayed metrics response from the previous case", async () => {
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({ frames: [RESET(4)], metricsHold: held });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.metricsCalls()).toBeGreaterThan(0));
    await selectCase(OTHER_CASE_ID, "Other case");

    release({ case_id: CASE_ID, session_id: SESSION_ID, device_count: 4242, event_count: 4242, alert_count: 4242, malformed_count: 0, updated_at: "2026-09-18T10:00:00Z" });
    await settle(80);

    // The previous case's counters must not appear under this one.
    expect(screen.queryByText("4242")).toBeNull();
  });

  it("keeps working after a request from a torn-down scope resolves", async () => {
    // Unmount cleanup is only complete if the page still functions
    // afterwards: fresh current-scope requests and stream frames must work.
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({ frames: [RESET(4)], alertHold: held });
    ...
    // A fresh mount is unaffected by the abandoned scope.
    const next = stubFetch({ frames: [sseFrame(95, "event.accepted", { event_id: "EV-AFTER-REMOUNT", observed_at: "2026-09-18T11:00:00Z", event_type: "telemetry" })] });
    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    expect(await screen.findByText("EV-AFTER-REMOUNT")).toBeTruthy();
    expect(next.calls.some(url => url.includes("/live/stream"))).toBe(true);
  });

  it("reports a failed rebuild as failed, not partial, when nothing succeeds", async () => {
    // Partial and failed are different facts: partial still restored
    // something, failed restored nothing. Collapsing them would tell an
    // investigator part of the view is authoritative when none of it is.
    const harness = stubFetch({ frames: [RESET(4)], failAll: true });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    await waitFor(() => expect(harness.eventCalls()).toBeGreaterThan(0));

    expect(await screen.findByText(/The rebuild failed\./i)).toBeTruthy();
    expect(screen.queryByText(/finished only in part/i)).toBeNull();
    expect(screen.queryByText(/Rebuilt from persisted state/i)).toBeNull();
    expect(screen.queryByRole("button", { name: "Retry rebuild" })).not.toBeNull();
  });

  it("reports a recovery buffer overflow and rebuilds again", async () => {
    // More records arrived during the rebuild than the buffer can hold, so
    // the merged feed would be quietly incomplete. That is reported and a
    // fresh authoritative rebuild is run, rather than claiming success.
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const flood = Array.from({ length: MAX_ALERTS_BOUND + 1 }, (_value, index) =>
      sseFrame(400 + index, "alert.created", {
        alert_id: `AL-FLOOD-${index}`, severity: "low", title: `FLOOD ${index}`,
        triggered_at: `2026-09-18T12:00:${String(index % 60).padStart(2, "0")}Z`,
      }));
    const harness = stubFetch({ frames: [RESET(4)], lateFrames: flood, alertHold: held });
    ...
    expect(await screen.findByText(/More records arrived during the rebuild than could be held/i)).toBeTruthy();
    expect(screen.queryByText(/Rebuilt from persisted state/i)).toBeNull();
    // A second authoritative rebuild is triggered, exactly once.
    await waitFor(() => expect(harness.alertCalls()).toBeGreaterThan(1));
    const afterRetry = harness.alertCalls();
    await settle(120);
    expect(harness.alertCalls()).toBe(afterRetry);
  });

  it("composes a paused rebuild with frames queued behind it on resume", async () => {
    // A reset reaches a paused view immediately, but frames behind it are
    // queued. Resuming must show both the snapshot and those frames.
    let release!: (value: unknown) => void;
    const held = new Promise(resolve => { release = resolve; });
    const harness = stubFetch({
      frames: [RESET(4)],
      lateFrames: [sseFrame(96, "event.accepted", { event_id: "EV-QUEUED-WHILE-PAUSED", observed_at: "2026-09-18T10:30:00Z", event_type: "telemetry" })],
      eventHold: held,
    });
    ...
    await waitFor(() => expect(harness.eventCalls()).toBeGreaterThan(0));
    await settle(80);
    // Queued, so not displayed yet.
    expect(screen.queryByText("EV-QUEUED-WHILE-PAUSED")).toBeNull();

    release({ items: [{ event_id: "EV-SNAPSHOT-WHILE-PAUSED", observed_at: "2026-09-18T09:30:00Z", event_type: "telemetry" }], page: 1, page_size: 1, total: 1 });
    await settle(80);
    expect(screen.queryByText("EV-SNAPSHOT-WHILE-PAUSED")).not.toBeNull();

    fireEvent.click(screen.getByRole("button", { name: "Resume display" }));
    await screen.findByRole("button", { name: "Pause display" });
    await settle(80);

    // Both compose: neither the snapshot nor the queued frame is lost.
    expect(screen.queryByText("EV-QUEUED-WHILE-PAUSED")).not.toBeNull();
    expect(screen.queryByText("EV-SNAPSHOT-WHILE-PAUSED")).not.toBeNull();
  });

  it("completes recovery when a newer timeline request supersedes the recovery one", async () => {
    // A `timeline.updated` frame during recovery aborts the recovery's own
    // timeline request. If the obligation to report that resource dies with
    // the aborted request, the rebuild sits on "Still loading: timeline"
    // forever even though the newest chronology is on screen. The transport
    // here rejects on abort, the way a real one does.
    //
    // Requests are held by shape rather than by call index: a recovery
    // timeline request carries no `analysis_id`, and the request a
    // `timeline.updated` frame starts carries one.
    const harness = stubFetch({
      frames: [RESET(4)],
      timelineHoldUnpinned: true,
      lateFrames: [sseFrame(97, "timeline.updated", { case_id: CASE_ID, analysis_id: "an-newer", entry_count: 1 })],
      timelinePages: [timelinePage(["Newest chronology"])],
    });

    render(<ConnectedLiveOperationsPage navigate={vi.fn()} />);
    // The superseding request is the one pinned to the named analysis.
    await waitFor(() => expect(harness.calls.some(url => url.includes("analysis_id=an-newer"))).toBe(true));

    expect(await screen.findByText("Newest chronology")).toBeTruthy();
    await waitFor(() => expect(screen.queryByText(/Rebuilt from persisted state/i)).not.toBeNull());
