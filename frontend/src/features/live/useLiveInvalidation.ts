...
export function useLiveInvalidation(options: {
  ...
  useEffect(() => {
    ...
    void source.connect({
      ...
      // Every callback is gated on this effect's own controller. Clearing
      // the queue when the scope changes is not enough on its own: a frame
      // read from the *previous* stream can be dispatched after the new
      // effect has already run, and since the queue is one ref shared
      // across scopes, that frame would be buffered under the new scope
      // and flushed into it on resume. Aborting is what makes the old
      // stream's callbacks inert, so a late frame is dropped rather than
      // re-homed.
      onState: state => { if (!controller.signal.aborted) stateHandler.current?.(state); },
      onError: message => { if (!controller.signal.aborted) errorHandler.current?.(message); },
      onMessage: message => {
